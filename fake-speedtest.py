...
def parse_args():
    ...
    description = (
        ...
        '------------------------------------------------------------'
        '--------------\n'
        'https://github.com/McDull-GitHub/fake-speedtest')
    ...
    parser.add_argument('--fake-download', default=None, help="Make a fake download record in Mbps to speedtest.net")
    parser.add_argument('--fake-upload', default=None, help="Make a fake upload record in Mbps to speedtest.net")
    parser.add_argument('--fake-ping', default=None, help="Make a fake ping record in ms to speedtest.net")
...
def shell():
    ...
    if not args.simple and args.share:
        if args.fake_download is not None:
            results.download = float(args.fake_download) * 1000000
        if args.fake_upload is not None:
            results.upload = float(args.fake_upload) * 1000000
        if args.fake_ping is not None:
            results.ping = float(args.fake_ping)
        results.share()
